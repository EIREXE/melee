...
/// One refraction curve: the modulus the distance is folded against, and the
/// scale applied afterwards. lbRefract_80021CE8() walks these as a flat array
/// of floats, two per entry.
typedef struct lbRefract_Param {
    /* 0x0 */ f32 modulus;
    /* 0x4 */ f32 scale;
} lbRefract_Param;

/// The `lbRefData` public symbol in LbRf.dat.
///
/// Declared with a plain pointer rather than MELEE_PC_PTR32: this is a DAT
/// *root*, so the PC build converts it out of file layout into host layout
/// (MELEE_PC_DAT below) instead of reading the file image in place. After that
/// the pointer is an ordinary host pointer and the floats are host order.
typedef struct lbRefract_Data {
    /* 0x0 */ u8 count;
    /* 0x4 */ lbRefract_Param* params; ///< [count]
} lbRefract_Data;
...
/// Completion callback for a card task. Named so the field below can carry a
/// pointer qualifier, which needs the type spelled before the declarator.
typedef void (*LbCardNewDone)(int);
