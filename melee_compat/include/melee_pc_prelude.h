...
// global header
// for some god-forsaken reason we use transitive includes from GC dolphin to
// provide some functions like memcpy i couldn't get the decomp to match when
// building for PPC without doing this... god forgive me Force-included
// (-include) into every translation unit of the PC build.
//
// due to aurora and dolphin's header structure being different the decomp
// actually expects to be able to transitivelly access some headers
//
...
// HSD_Synth_804D6018 leaves an uninitialised audio heap handle, so this is
// able to tolerate it
...
// defers the completion callback instead of running it inline, the reason
// being that melee expects to set a pending flag afterwards, which aurora on't
// do because it's synchronous, leading to a deadlock
struct CARDFileInfo;
struct CARDStat;
s32 melee_pc_card_read_async(const struct CARDFileInfo*, void*, s32, s32,
                             void (*)(s32, s32));
s32 melee_pc_card_write_async(const struct CARDFileInfo*, const void*, s32,
                              s32, void (*)(s32, s32));
